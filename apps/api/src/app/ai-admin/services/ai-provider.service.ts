...
export class AiProviderService {
    ...
    async testConnection(id: string): Promise<{ success: boolean; message: string; latencyMs?: number }> {
        ...
        try {
            ...
            if (provider.type === 'ollama') {
                // For Ollama, we can try a simple request
                ...
                // For vLLM or OpenAI, try listing models
    ...
    createOpenAIClient(provider: AiProvider): OpenAI {
        ...
        // Normalize endpoint for Ollama
        if (provider.type === 'ollama' && !baseURL.includes('/v1')) {
            baseURL = baseURL.replace(/\/$/, '') + '/v1';
        }

        return new OpenAI({
            apiKey: provider.apiKey || 'ollama', // Ollama doesn't need a real key
            baseURL,
